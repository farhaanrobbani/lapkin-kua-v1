...
// Reusable Telegram HTML Sender with Retry
...
router.post('/webhook', async (req: Request, res: Response) => {
  ...
  if (update && update.message) {
    ...
    if (text.startsWith('/start')) {
      ...
      inlineKeyboard = {
        inline_keyboard: [
          [{ text: '📊 Laporan Hari Ini', callback_data: '/today' }, { text: '📅 Jadwal Mendatang', callback_data: '/upcoming' }],
          [{ text: '📑 Status Dokumen', callback_data: '/dokumen' }, { text: '💰 Rekap Tukin', callback_data: '/pembayaran' }]
        ]
      };
    } else if (text.startsWith('/today')) {
      const todayData = db.getKuaDailyByDate(todayStr);
      if (todayData) {
        replyText = `<b>📊 LAPORAN DAILY KUA - ${todayStr}</b>\n\n` +
      ...
      replyText = `<b>🚗 STATUS KENDARAAN OPERASIONAL KUA</b>\n\n` +
      ...
      replyText = `<b>📑 STATUS KELENGKAPAN DOKUMEN & LAPORAN</b>\n\n` +
      ...
      replyText = `<b>💰 REKAPITULASI TUNJANGAN KINERJA (TUKIN)</b>\n\n` +
...
// POST /api/telegram/simulate-command
router.post('/simulate', async (req: Request, res: Response) => {
  const { command, chatId } = req.body;
  const targetChatId = chatId || '123456789';

  // Trigger internal logic directly for UI testing
  ...
  db.addTelegramLog({
    ...
    user_name: 'Pengguna Simulasi Web',
    message: responseMsg,
    status: 'sent'
  });
